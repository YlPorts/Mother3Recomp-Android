...
public final class Mother3SetupActivity extends GbaSetupActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        getSharedPreferences("setup", MODE_PRIVATE).edit()
            .putBoolean("skip_launcher_on_boot", false).apply();
    ...
    private void showNativeLogIfChanged() {
        ...
        String text;
        try { text = readTail(log, 12000); } catch (IOException error) { return; }
        if (text.trim().isEmpty()) return;
        String previous = getSharedPreferences(PREFS, MODE_PRIVATE).getString(LAST_LOG, "");
        if (text.equals(previous)) return;
        getSharedPreferences(PREFS, MODE_PRIVATE).edit().putString(LAST_LOG, text).apply();
        new AlertDialog.Builder(this)
            .setTitle("MOTHER 3 — último arranque")
            .setMessage(lastLines(text, 45))
